/* ================================================================
   ...
   WebExtension tabs and storage APIs directly — no postMessage bridge needed.

   What this file does:
   1. Reads open browser tabs directly via tabs.query()
   2. Groups tabs by domain with a landing pages category
   3. Renders domain cards, banners, and stats
   4. Handles all user actions (close tabs, save for later, focus tab)
   5. Stores "Saved for Later" tabs in storage.local (no server)
   ================================================================ */
...
if (!extensionApi) {
  throw new Error('Tab Out requires WebExtension APIs.');
}
...
function afterNextPaint() {
  ...
  // Double rAF: the first callback runs before the next paint, and the second
  // gives the browser one completed frame with the greeting before tabs mount.
...
/* ----------------------------------------------------------------
   WEBEXTENSION TABS — Direct API Access

   Since this page IS the extension's new tab page, it has full
   access to tabs and storage. No middleman needed.
   ---------------------------------------------------------------- */
...
function isInternalBrowserUrl(url) {
  return parseBrowserUrl(url).isInternal;
}

function parseBrowserUrl(url) {
  if (!url) return { isInternal: true, protocol: '', hostname: '', pathname: '' };
  try {
    ...
    return { isInternal: true, protocol: '', hostname: '', pathname: '' };
  }
}

function isRealTab(tab) {
  if (!tab) return false;
  if (typeof tab.isInternal === 'boolean') return !tab.isInternal;
...
function tabFavicon(tab) {
  return tab && tab.favIconUrl && !isInternalBrowserUrl(tab.favIconUrl) ? tab.favIconUrl : '';
}

/**
 * fetchOpenTabs()
 *
 * Reads all currently open browser tabs directly from the browser.
 * Sets the extensionId flag so we can identify Tab Out's own pages.
 */
async function fetchOpenTabs() {
  try {
    const newtabUrl = extensionApi.runtime.getURL('index.html');

    const tabs = await extensionApi.tabs.query({});
    openTabs = tabs.map(t => {
      const urlInfo = parseBrowserUrl(t.url);
      return {
        id:       t.id,
        url:      t.url,
        title:    t.title,
    ...
    // Tabs API unavailable (shouldn't happen in an extension page)
    openTabs = [];
  }
}
...
async function closeTabsByUrls(urls) {
  ...
  if (toClose.length > 0) await extensionApi.tabs.remove(toClose);
  await fetchOpenTabs();
}
...
async function closeTabOutDupes() {
  ...
  const tabOutTabs = allTabs.filter(t =>
    t.url === newtabUrl || t.url === 'chrome://newtab/' || t.url === 'about:newtab'
  );
 ...
 * saveTabForLater(tab)
 *
 * Saves a single tab to the "Saved for Later" list in storage.local.
 * @param {{ url: string, title: string }} tab
 */
async function saveTabForLater(tab) {
  const { deferred = [] } = await extensionApi.storage.local.get('deferred');
  deferred.push({
  ...
  await extensionApi.storage.local.set({ deferred });
}

/**
 * getSavedTabs()
 *
 * Returns all saved tabs from storage.local.
...
const GREETING_ART = {
  'Good morning': 'assets/greetings/good-morning.png',
  'Good afternoon': 'assets/greetings/good-afternoon.png',
  'Good evening': 'assets/greetings/good-evening.png',
};
 ...
 * getRealTabs()
 *
 * Returns tabs that are real web pages — no browser internals, extension
 * pages, about:blank, etc.
 */
function getRealTabs() {
  return openTabs.filter(isRealTab);
}
...
function renderDomainCard(group, groupIndex = 0) {
  ...
  const revealDelay = Math.min(groupIndex * 36, 540);
  ...
  const dupeBadge = hasDupes
    ? `<span class="open-tabs-badge dupe-tabs-badge">
 ...
 *
 * Reads saved tabs from storage.local and renders the right-side
...
function renderDeferredItem(item) {
  ...
  const safeFavIconUrl = escapeHtml(!isInternalBrowserUrl(item.favIconUrl) ? item.favIconUrl : '');
    ...
    <div class="deferred-item" data-deferred-id="${escapeHtml(item.id)}">
      ...
      <div class="deferred-info">
        <a href="${safeUrl}" target="_blank" rel="noopener" class="deferred-title" title="${safeTitle}">
          ${safeFavIconUrl ? `<img src="${safeFavIconUrl}" alt="" loading="lazy" decoding="async" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px">` : ''}${safeTitle}
        </a>
 ...
 * 2. Fetches open tabs via tabs.query()
...
async function renderStaticDashboard() {
  ...
  let greetingReady = Promise.resolve();

  if (greetingEl) {
    const greeting = getGreeting();
    const image = document.createElement('img');
    image.className = 'greeting-art';
    image.src = GREETING_ART[greeting];
    image.alt = greeting;
    image.decoding = 'async';
    image.loading = 'eager';
    image.fetchPriority = 'high';
    greetingEl.replaceChildren(image);
    greetingReady = waitForImageReady(image);
  }
  if (dateEl)     dateEl.textContent     = getDateDisplay();

  // Keep the saved-tabs column in its final right-side position during the
  // first paint, while the real tab cards wait for the greeting frame.
  if (openTabsSection) {
  ...
  // --- Fetch tabs + optional personal config in parallel ---
  // config.local.js is intentionally optional; loading it here avoids a
  // parser-blocking 404 before the dashboard can even start.
  const tabsReady = fetchOpenTabs();
  const configReady = loadOptionalLocalConfig();
  await Promise.all([tabsReady, configReady, greetingReady]);
  ...
  function isLandingPage(tab) {
    ...
    return LANDING_PAGE_PATTERNS.some(p => {
      ...
      const hostnameMatch = p.hostname
        ? hostname === p.hostname
        : p.hostnameEndsWith
          ? hostname.endsWith(p.hostnameEndsWith)
          : false;
      if (!hostnameMatch) return false;
      if (p.test)       return p.test(pathname, tab.url);
      if (p.pathPrefix) return pathname.startsWith(p.pathPrefix);
      if (p.pathExact)  return p.pathExact.includes(pathname);
      return pathname === '/';
    });
  }
  ...
  for (const tab of realTabs) {
    try {
      if (isLandingPage(tab)) {
        landingTabs.push(tab);
        continue;
      }

      // Check custom group rules first (e.g. merge subdomains, split by path)
      const customRule = matchCustomGroup(tab);
      if (customRule) {
  ...
  if (domainGroups.length > 0 && openTabsSection) {
    ...
    if (openTabsMissionsEl) openTabsMissionsEl.classList.remove('stagger-in');
...
document.addEventListener('click', async (e) => {
  ...
  if (action === 'close-single-tab') {
    ...
    // Close the tab directly through the WebExtension tabs API
    const allTabs = await extensionApi.tabs.query({});
    const match   = allTabs.find(t => t.url === tabUrl);
    if (match) await extensionApi.tabs.remove(match.id);
    await fetchOpenTabs();
  ...
  if (action === 'defer-single-tab') {
    ...
    try {
      await saveTabForLater({
        url: tabUrl,
        title: tabTitle,
        favIconUrl: actionEl.dataset.tabFavicon || '',
      });
    } catch (err) {
    ...
    // Close the tab through the WebExtension tabs API
    const allTabs = await extensionApi.tabs.query({});
    const match   = allTabs.find(t => t.url === tabUrl);
    if (match) await extensionApi.tabs.remove(match.id);
    await fetchOpenTabs();
